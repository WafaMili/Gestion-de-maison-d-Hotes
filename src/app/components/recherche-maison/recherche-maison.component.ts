...
export class RechercheMaisonComponent implements OnInit {
  ...
  search() {
    ...
    this.searchedMaisons = this.maisonTable.filter(maison =>
      maison.nomMaison.toLowerCase().includes(this.searchByName.toLowerCase()) &&
      maison.adresse.toLowerCase().includes(this.searchByAddress.toLowerCase())
    );
  }
  ngOnInit() {
  }
